            ...
            {close && position && pos &&
                <>
                    <aside style={{ transform: `translateX(${!show ? pos : 0})` }} className={`fixed ${position === "left" ? "left-0" : "right-0"}  top-0 z-[1000] h-screen transition duration-150 ease-out`}>
                        ...
                        <div className="flex flex-col custom_100vh bg-white drop-shadow-3xl w-[100vw] max-w-[270px] lg:max-w-[350px]  ">
                    ...
                    <aside
                        onClick={() => close(false)}
                        className={`fixed ${!show && "hidden"}  z-[999] blur-sm top-0 left-0 w-screen custom_100vh bg-[rgba(0,0,0,.1)] transition duration-250 ${show ? "opacity-100" : "opacity-0"}`}
                    />
                </>
            }
        </>
